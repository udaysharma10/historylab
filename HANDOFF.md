...
- **Mockups DONE (`mockups/launch-0{0-5}*.html`, open `launch-00-index.html`):** landing+waitlist, invite signup (code→Google→DPDPA consent), locked-Ch2→purchase→success/pending, account+3 invites, admin panel (users/grants/invite-batches/waitlist/purchases). All in the C2 design language; dashed boxes = spec annotations tied to PLAN_SUBSCRIPTION_LAUNCH.md. Verified via headless-Chrome screenshots. Key design intents: locked premium card looks desirable (warm, not greyed); admin grants render identically to purchases for the student; no teacher role anywhere.
