...
### PRODUCT / MONETISATION (planning only — not started)
- **`PLAN_SUBSCRIPTION_LAUNCH.md`** (new, 2026-06-28) = the technical plan to launch as a paid subscription portal. Builds on `BIGGER_PICTURE.md` (freemium strategy: Free Ch1 / Plus ₹499/yr / Pro ₹999/yr) + `PLAN_AUTH_AND_ANALYTICS.md` (Supabase auth done; progress-sync planned, **not implemented** — stores are localStorage-only).
- **Key insight captured there:** content currently ships in the JS bundle, so a real paywall must **move premium chapters server-side** (Supabase Edge Function + entitlement check); a UI toggle isn't enough. Stack = Vercel + Supabase + **Razorpay** subscriptions/webhooks. Don't-skip: DPDPA (minors → parental consent), GST invoices, refund/ToS pages.
