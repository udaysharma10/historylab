...
interface SourceQuizCardProps {
  activity: SourceComprehensionActivity
  source: SourceBox
  sectionColor: string
...
export function SourceQuizCard({
  activity,
  source,
  sectionColor,
  questionNumber,
  ...
  const advanceQuestion = (addedCorrect: number) => {
    if (currentQ < activity.questions.length - 1) {
      ...
      onComplete(correctCount + addedCorrect, activity.questions.length)
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="p-5 sm:p-6">
        ...
        <div className="bg-amber-50/60 border border-amber-200/50 rounded-xl p-3 mb-5 max-h-24 overflow-y-auto">
          <p className="font-body text-xs text-hist-dark/70 leading-relaxed italic line-clamp-3">
