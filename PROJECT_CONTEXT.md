...
│   │   │   ├── NarrativeCard.tsx        # Card renderer: text (drop cap), vocabulary (purple), source (amber), figure (now with INTERACTIVE clickable hotspots on the image + clickable legend tags), flowchart (clickable pop-up step boxes), table (rendered HTML table), timeline-ref, map-ref, exam-prep
...
│   └── modules/
│       ├── HomePage.tsx                 # Chapter home: greeting, stats, 6 section cards, "Study Tools" cluster (chapter-level revision modalities: Timeline/Maps/Figures/Flashcards/Exam Prep — relabelled from "Learning Modes"/"X Review")
...
│       ├── SectionModule.tsx            # BUILT: Section landing page — "What You'll Learn", prominent top "Start/Continue Reading" CTA, then Topics list, Practice Quiz, and in-section Timeline/Maps (revision) below the lesson
...
**Extraction method (current, clean)**: `pdfimages -png -p <PDF>` extracts the **embedded original figure images** directly — clean, no page text/captions/QR. Match each embedded image to its figure by **NCERT page number** (filename `img-<page>-<obj>.png`); skip the per-page template layers (2480×3508 gray, 1894×1894 QR). Then RGB-convert + downscale to ≤1300px. This **superseded** the older `pdftoppm` page-render + crop method, which had baked NCERT body text / "Discuss"/"Activity" boxes / captions / the page QR code into many figure files (and thereby broke hotspot %-coordinates). All Ch1 (20) and Ch2 (15) figures were re-extracted this way; the public-domain Jallianwala (fig-15) & Dyer (fig-16) images were sourced from Wikimedia Commons instead (not in the NCERT PDF).

**Hotspots**: `{x,y}` are **percent of the displayed figure image**. They are now placed against the clean images and verified with a coordinate-grid overlay (Ch1 ~64 + Ch2 ~33 dots). When swapping a figure image, hotspot coords MUST be re-checked. Ch1 hotspot order in `figures.ts` is `x, y, label`; Ch2 order is `label, description, x, y`.
...
| CR-1..8 | **Content Review (Neha) Batches 1-8** — see `CONTENT_REVIEW_PROCESS.md` log. Added reusable **`flowchart`** (clickable pop-up steps) + **`table`** narrative card types; interactive clickable hotspots in narrative figure cards; precise centre-pin markers on InteractiveMap. **Ch1:** NCERT headings, vocab-before-use reordering, French-measures/Napoleonic-Code/Zollverein/Treaty-of-Vienna/unification (Germany/Italy/Britain) flowcharts, Habsburg + society tables, Marianne symbols + re-extracted clearer fig-16 stamp, Greek-war & Balkans-WWI exam cards, MCQ answer-position rebalance (was all option B). **Ch2:** WWI impact moved to chapter start + NCERT-framed Rowlatt, Satyagraha simplified + 3-satyagraha flowchart, martial-law explained, NCM-participants recap as real table, Dandi-march pointers, Ch2-S4 figure fix. Jallianwala Bagh + General Dyer figures added (public-domain, Wikimedia). | **DONE** |
| CR-9 | **Figure & UX correctness round (Neha + Uday review).** (1) **All 35 figure images re-extracted clean** via `pdfimages` — root cause of "misaligned dots" was page-text/QR baked into figure files; (2) **all ~97 hotspots remapped + grid-verified** against the clean frames; (3) chapter-level **"Learning Modes" → "Study Tools"** (plain tile names, was "X Review"); (4) section page **"Start Reading" moved to a prominent top CTA** (was buried under quiz/timeline/maps). **OPEN (full UX track, not yet done):** verify chapter-home Study-Tools tile count (Ch1 should show 5); "Topics" list mixes Practice Quiz with lesson topics; full screen-by-screen UX/IA + mobile pass. | **DONE (UX track pending)** |
